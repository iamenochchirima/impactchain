...
import { MarginalizedCommunitySupportData as  MarginalizedCommunitySupportDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const  MarginalizedCommunitySupportData = ({ setManualData, setUploadManually }) => {
  ...
  const [typeOfCommunity, setTypeOfCommunity] = useState<string>("");
  const [totalFunding, setTotalFunding] = useState<string>("");

  const [programs, setPrograms] = useState< MarginalizedCommunitySupportDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one marginalized community support program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving marginalized community support program", error);
    }
  };
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Marginalized Community Support Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Marginalized Community Support program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the specific name of the program supporting marginalized communities."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of community is supported?</label>
            <select
              className={styles.formSelectInput}
              id="typeOfCommunity"
              value={typeOfCommunity}
              onChange={(e) => setTypeOfCommunity(e.target.value)}
              required
            >
              <option value="">Select the type of marginalized community the program supports from the dropdown.</option>
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>How many communities are supported?</label>
            <input
              className={styles.formInput}
              id="totalFunding"
              type="number"
              placeholder="Enter the total number of communities supported by this program."
              value={totalFunding}
...
export default MarginalizedCommunitySupportData;