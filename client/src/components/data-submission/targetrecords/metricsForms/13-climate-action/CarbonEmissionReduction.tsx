...
import { CarbonEmissionReductionData as CarbonEmissionReductionDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const CarbonEmissionReductionData = ({ setManualData, setUploadManually }) => {
  ...
  const [typeOfEmissionReduced, setTypeOfEmissionReduced] = useState<string>("");
  const [reductionInEmission, setReductionInEmission] = useState<string>("");

  const [programs, setPrograms] = useState<CarbonEmissionReductionDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one carbon emission reduction program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving carbon emission reduction program", error);
    }
  };
  ...
  const handleSave = async () => {
    ...
    setDuration("");
    setLocation("");
    setTypeOfEmissionReduced("");
    setReductionInEmission("");
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Carbon Emission Reduction Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Carbon Emission Reduction program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the specific name of the carbon emission reduction initiative."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of emission is reduced?</label>
            <select
              className={styles.formSelectInput}
              id="typeOfEmissionReduced"
              value={typeOfEmissionReduced}
              onChange={(e) => setTypeOfEmissionReduced(e.target.value)}
              required
            >
              <option value="">Select the type of emission targeted by the reduction initiative from the dropdown.</option>
              <option value="CO2">CO2</option>
              <option value="methane">Methane</option>
              <option value="nitrous-oxide">Nitrous Oxide</option>
              <option value="f-gases">F-gases</option>
            </select>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the amount in emission reduction</label>
            <input
              className={styles.formInput}
              id="reductionInEmssion"
              type="number"
              placeholder="Specify the volume of emissions reduced by this initiative."
              value={reductionInEmission}
              onChange={(e) => setReductionInEmission(e.target.value)}
              required
            />
          </div>
...
export default CarbonEmissionReductionData;
