...
import { RenewableEnergyInvestmentData as RenewableEnergyInvestmentDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const RenewableEnergyInvestmentData = ({ setManualData, setUploadManually }) => {
  ...
  const [typeOfRenewableEnergyInvestment, setTypeOfRenewableEnergyInvestment] = useState<string>("");
  const [amountInvested, setAmountInvested] = useState<string>("");
  

  const [programs, setPrograms] = useState<RenewableEnergyInvestmentDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one renewable energy investment program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving renewable energy investment program", error);
    }
  };
  ...
  const handleSave = async () => {
    ...
    setDuration("");
    setLocation("");
    setTypeOfRenewableEnergyInvestment("");
    setAmountInvested("");
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Renewable Energy Investment Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Renewable Energy Investment program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the name of the renewable energy investment program."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of renewable energy investment is provided?</label>
            <select
              className={styles.formSelectInput}
              id="typeOfRenewableEnergyInvestment"
              value={typeOfRenewableEnergyInvestment}
              onChange={(e) => setTypeOfRenewableEnergyInvestment(e.target.value)}
              required
            >
              <option value="">Select the type of renewable energy investment from the dropdown.</option>
          ...
          <div className={styles.inputDiv}>
            ...
            <input
              ...
              id="amountInvested"
              type="number"
              placeholder="Enter the total amount invested in renewable energy projects."
...
export default RenewableEnergyInvestmentData;
