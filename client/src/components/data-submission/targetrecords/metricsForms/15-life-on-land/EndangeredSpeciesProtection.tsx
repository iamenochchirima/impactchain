...
import { EndangeredSpeciesProtectionData as EndangeredSpeciesProtectionDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const EndangeredSpeciesProtectionData = ({ setManualData, setUploadManually }) => {
  ...
  const [typeOfSpecies, setTypeOfSpecies] = useState<string>("");
  const [numberOfSpeciesProtected, setNumberOfSpeciesProtected] = useState<string>("");

  const [programs, setPrograms] = useState<EndangeredSpeciesProtectionDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one endangered species protection program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving endangered species protection program", error);
    }
  };
  ...
  const handleSave = async () => {
    ...
    // const newProgram: EndangeredSpeciesProtectionDataType = {
    //   programName,
    //   startDate:BigInt(new Date(startDate).getTime()),
    //   duration,
    //   location,
    //   typeOfSpecies,
    //   numberOfSpeciesProtected: BigInt(numberOfSpeciesProtected),
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Endangered Species Protection Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Endangered Species Protection program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the name of the endangered species protection program."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of species is protected?</label>
            <select
              className={styles.formSelectInput}
              id="typeOfSpecies"
              value={typeOfSpecies}
              onChange={(e) => setTypeOfSpecies(e.target.value)}
              required
            >
              <option value="">Select the type of species being protected from the dropdown.</option>
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>How many species are protected?</label>
            <input
              className={styles.formInput}
              id="numberOfSpeciesProtected"
              type="number"
              placeholder="Enter the number of species protected under this initiative."
              value={numberOfSpeciesProtected}
              onChange={(e) => setNumberOfSpeciesProtected(e.target.value)}
              required
            />
          </div>
...
export default EndangeredSpeciesProtectionData;
