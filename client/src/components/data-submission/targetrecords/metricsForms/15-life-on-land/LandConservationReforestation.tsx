...
import {LandConservationReforestationData as LandConservationReforestationDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const LandConservationReforestationData = ({ setManualData, setUploadManually }) => {
  ...
  const [typeOfLandConservation, setTypeOfLandConservation] = useState<string>("");
  const [areaOfLandConserved, setAreaOfLandConserved] = useState<string>("");

  const [programs, setPrograms] = useState<LandConservationReforestationDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one land conservation reforestation program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving land conservation reforestation program", error);
    }
  };
  ...
  const handleSave = async () => {
    ...
    setDuration("");
    setLocation("");
    setTypeOfLandConservation("");
    setAreaOfLandConserved("");
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Land Conservation Reforestation Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Land Conservation Reforestation program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the specific name of the land conservation and reforestation project."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of land conservation is involved?</label>
            <select
              className={styles.formSelectInput}
              id="typeOfLandConservation"
              value={typeOfLandConservation}
              onChange={(e) => setTypeOfLandConservation(e.target.value)}
              required
            >
              <option value="">Select the type of land involved in the conservation efforts from the dropdown.</option>
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the area of the Land Conserved?</label>
            <input
              className={styles.formInput}
              id="areaOfLandConserved"
              type="number"
              placeholder="Specify the area of land conserved by this project."
              value={areaOfLandConserved}
              onChange={(e) => setAreaOfLandConserved(e.target.value)}
              required
            />
          </div>
...
export default LandConservationReforestationData;
