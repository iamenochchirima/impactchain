...
import { AntiCorruptionProgramsData as AntiCorruptionProgramsDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const AntiCorruptionProgramsData = ({ setManualData, setUploadManually }) => {
  ...
  const [natureOfAntiCorruptionMeasures, setNatureAntiCorruptionMeasures] = useState<string>("");

  const [programs, setPrograms] = useState<AntiCorruptionProgramsDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one anti-corruption program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving anti-corruption program", error);
    }
  };
  ...
  const handleSave = async () => {
    ...
    if(
      ...
      location === "" ||
      typeOfProgram === "" ||
      natureOfAntiCorruptionMeasures === "" 
    ...
    // const newProgram: AntiCorruptionProgramsDataType = {
    //   programName,
    ...
    setLocation("");
    setTypeOfProgram("");
    setNatureAntiCorruptionMeasures("");
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Anti Corruption Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Anti Corruption program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the specific name of the anti-corruption program."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of program is measured?</label>
            <select
              ...
              required
            >
              <option value="">Select the type of anti-corruption measures implemented from the dropdown.</option>
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the nature of the program?</label>
            <input
              className={styles.formInput}
              id="natureOfAntiCorruptionMeasures"
              type="text"
              placeholder="Explain the reason of the program"
              value={natureOfAntiCorruptionMeasures}
              onChange={(e) => setNatureAntiCorruptionMeasures(e.target.value)}
              required
            />
          </div>
...
export default AntiCorruptionProgramsData;
