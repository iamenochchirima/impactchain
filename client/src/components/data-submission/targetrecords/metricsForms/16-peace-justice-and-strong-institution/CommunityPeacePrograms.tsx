...
import { CommunityPeaceProgramsData as CommunityPeaceProgramsDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const CommunityPeaceProgramsData = ({ setManualData, setUploadManually }) => {
  ...
  const [numberOfPrograms, setNumberOfPrograms] = useState<string>("");

  const [programs, setPrograms] = useState<CommunityPeaceProgramsDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one community peace program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving community peace program", error);
    }
  };
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Community Peace Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Community Peace program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the name of the community peace program."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of peace initiative is implemented?</label>
            <select
              ...
              required
            >
              <option value="">Select the type of peace initiative from the dropdown.</option>
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>How many programs are conducted?</label>
            <input
              className={styles.formInput}
              id="numberOfPrograms"
              type="number"
              placeholder="Enter the number of community peace programs conducted under this initiative."
              value={numberOfPrograms}
              onChange={(e) => setNumberOfPrograms(e.target.value)}
              required
            />
          </div>
...
export default CommunityPeaceProgramsData;
