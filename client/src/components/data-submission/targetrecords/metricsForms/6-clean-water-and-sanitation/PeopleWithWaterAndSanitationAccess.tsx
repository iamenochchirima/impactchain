...
import { WaterSanitationAccessData as WaterSanitationAccessDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const WaterSanitationAccessData = ({ setManualData, setUploadManually }) => {
  ...
  const [totalFundsAllocated, setTotalFundsAllocated] = useState<string>("");
  const [numberOfBeneficiaries, setNumberOfBeneficiaries] = useState<string>("");

  const [programs, setPrograms] = useState<WaterSanitationAccessDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one water and sanitation access program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving water and sanitation access program", error);
    }
  };
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Water and Sanitation Access Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Water and Sanitation Access program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the name of the program providing water and sanitation access."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            ...
            <input
              ...
              id="startDate"
              type="date"
              placeholder="Indicate when the program started providing access."
              value={startDate}
              required
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>How many individuals are involved?</label>
            <input
              className={styles.formInput}
              id="numberOfBeneficiaries"
              type="number"
              placeholder="Enter the total number of individuals who have gained access to water and sanitation through this program."
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the total amount of funds allocated? (In ZAR)</label>
            <input
              className={styles.formInput}
              id="totalFundsAllocated"
              type="number"
              placeholder="Enter the total amount of funds allocated to this program."
              value={totalFundsAllocated}
              onChange={(e) => setTotalFundsAllocated(e.target.value)}
              required
            />
          </div>
...
export default WaterSanitationAccessData;
