...
import { SanitationFacilitiesData as SanitationFacilitiesDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const SanitationFacilitiesData = ({ setManualData, setUploadManually }) => {
  ...
  const [typeOfFacility, setTypeOfFacility] = useState<string>("");
  const [totalInvestment, setTotalInvestment] = useState<string>("");

  const [programs, setPrograms] = useState<SanitationFacilitiesDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one sanitation facility program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving sanitation facility program", error);
    }
  };
  ...
  const handleSave = async () => {
    ...
    const newProgram: SanitationFacilitiesDataType = {
      programName,
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Sanitation Facility Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Sanitation Facility program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the specific name of the sanitation facilities program."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of facilities are there?</label>
            <select
              ...
              required
            >
              <option value="">Select the primary type of sanitation facility developed from the dropdown.</option>
              <option value="financial">Public Toilets</option>
              <option value="resource-based">Community Bathrooms</option>
              <option value="consultation">Septic Systems</option>
              <option value="other">Seweragee Systems</option>
            </select>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the total invested?</label>
            <input
              className={styles.formInput}
              id="totalInvestment"
              type="number"
              placeholder="Enter the total amount of funding invested"
