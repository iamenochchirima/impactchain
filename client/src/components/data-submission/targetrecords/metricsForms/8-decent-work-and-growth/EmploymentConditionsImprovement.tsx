...
import { EmploymentConditionsImprovementData as EmploymentConditionsImprovementDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const EmploymentConditionsImprovementData = ({ setManualData, setUploadManually }) => {
  ...
  const [typeOfImprovement, setTypeOfImprovement] = useState<string>("");
  const [numberOfBeneficiaries, setNumberOfBeneficiaries] = useState<string>("");

  const [programs, setPrograms] = useState<EmploymentConditionsImprovementDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one employment conditions improvement program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving employment conditions improvement program", error);
    }
  };
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Employment Conditions Improvement Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Employment Conditions Improvement program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the name of the employment or work condition improvement initiative."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            ...
            <input
              ...
              id="startDate"
              type="date"
              placeholder="Indicate when the improvements began."
              value={startDate}
              required
          ...
          <div className={styles.inputDiv}>
            ...
            <input
              ...
              id="location"
              type="text"
              placeholder="Specify the city and country where the improvements are implemented."
              value={location}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of improvement considered?</label>
            <select
              className={styles.formSelectInput}
              id="typeOfImprovement"
              value={typeOfImprovement}
              onChange={(e) => setTypeOfImprovement(e.target.value)}
              required
            >
              <option value="">Select the type of improvement from the dropdown.</option>
              <option value="health-and-safety">Health and Safety</option>
              <option value="wage-increase">Wage Increase</option>
              <option value="working-hours">Working Hours</option>
              <option value="other">Other</option>
            </select>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>How many individuals are affected?</label>
            <input
              className={styles.formInput}
              id="numberOfBeneficiaries"
              type="number"
              placeholder="Enter the number of individuals affected by the improvements."
...
export default EmploymentConditionsImprovementData;
