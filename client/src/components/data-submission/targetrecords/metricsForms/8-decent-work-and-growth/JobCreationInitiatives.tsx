...
import { JobCreationInitiativesData as JobCreationInitiativesDataType, Testimonials } from "../../../../../hooks/declarations/data/data.did";
...
const JobCreationInitiativesData = ({ setManualData, setUploadManually }) => {
  ...
  const [jobsSectors, setJobsSectors] = useState<string>("");

  const [programs, setPrograms] = useState<JobCreationInitiativesDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one job creation initiative program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving job creation initiative program", error);
    }
  };
  ...
  return (
    <div>
       <div className="">
        <h3 className={styles.title}>Job Creation Initiative</h3>
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Job Creation Initiative Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Job Creation Initiative program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the specific name of the job creation initiative."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What are the job sectors?</label>
            <select
              className={styles.formSelectInput}
              id="jobsSector"
              value={jobsSectors}
              onChange={(e) => setJobsSectors(e.target.value)}
              required
            >
              <option value="">Select the primary sector involved in the job creation initiative from the dropdown.</option>
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>WThe amount of jobs created?</label>
            <input
              className={styles.formInput}
              id="jobsCreated"
              type="number"
              placeholder="Enter the total number of jobs created by this initiative."
              value={jobsCreated}
...
export default JobCreationInitiativesData;
