...
import { PeopleBenefitingFromInfrastructureData as PeopleBenefitingFromInfrastructureDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const PeopleBenefitingFromInfrastructureData = ({ setManualData, setUploadManually }) => {
  ...
  const [programs, setPrograms] = useState<PeopleBenefitingFromInfrastructureDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one People Benefiting from Infrastructure program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving People Benefiting from Infrastructure program", error);
    }
  };
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a People Benefiting from Infrastructure Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the People Benefiting from Infrastructure program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the name of the infrastructure benefit initiative."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>How many individuals benefit from this initiative?</label>
            <input
              className={styles.formInput}
              id="totalBeneficiaries"
              type="number"
              placeholder="Enter the number of people benefiting from the infrastructure."
...
export default PeopleBenefitingFromInfrastructureData;
