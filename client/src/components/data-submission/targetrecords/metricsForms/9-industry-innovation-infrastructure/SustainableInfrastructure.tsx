...
import { SustainableInfrastructureData as SustainableInfrastructureDataType } from "../../../../../hooks/declarations/data/data.did";
import { ManualData } from "../../MetricRecords";
import { IoMdAdd } from "react-icons/io";
import Program from "../support/Program";

const SustainableInfrastructureData = ({ setManualData, setUploadManually }) => {
  ...
  const [typeOfInfrastructure, setTypeOfInfrastructure] = useState<string>("");

  const [programs, setPrograms] = useState<SustainableInfrastructureDataType[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    if (programs.length === 0) {
      toast.error("Please add at least one sustainable infrastructure program you did", {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      console.log("Error saving sustainable infrastructure program", error);
    }
  };
  ...
  const handleSave = async () => {
    ...
    const newProgram: SustainableInfrastructureDataType = {
      programName,
        ...
        <div className={styles.munualDataForm}>
          <div className={styles.formHeader}>
            <h3 className={styles.formTitle}>
              Add a Sustainable Infrastructure Program
            </h3>
          </div>
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What is the name of the Sustainable Infrastructure program?</label>
            <input
              className={styles.formInput}
              id="programName"
              type="text"
              placeholder="Enter the name of the sustainable infrastructure development project."
              value={programName}
          ...
          <div className={styles.inputDiv}>
            ...
            <input
              ...
              id="location"
              type="text"
              placeholder="Specify the city and country where the project is located."
              value={location}
          ...
          <div className={styles.inputDiv}>
            <label htmlFor={styles.inputLabel}>What type of infrastructure is provided?</label>
            <select
              className={styles.formSelectInput}
              id="typeOfInfrastructure"
              value={typeOfInfrastructure}
              onChange={(e) => setTypeOfInfrastructure(e.target.value)}
              required
            >
              <option value="">Select the type of infrastructure from the dropdown.</option>
...
export default SustainableInfrastructureData;