...
import MetricItem from "../../../../../components/data-submission/metrics/MetricItem";
import { RootState } from "../../../../../redux/store";
import { setLocalImpactTargets, setLocalRecord } from "../../../../../redux/slices/app";
...
const TargetsMetricCard: FC<Props> = ({
  target,
}) => {
  const {localImpactTargets, localRecord} = useSelector((state: RootState) => state.app);
  ...
  useEffect(() => {
      if (!localRecord || !localImpactTargets) return 
      ...
      const updatedTargets = localImpactTargets.map((t) =>
        t.id === target.id ? updatedTarget : t
      );
      dispatch(setLocalImpactTargets(updatedTargets));
      const updatedImpactTargets = { ...localRecord.impactTargets };
      ...
      dispatch(setLocalRecord(updatedRecord));
  }, [selectedMetrics]);
...
export default TargetsMetricCard;
